...
void show(char[] referenceList, char[][] authors)
{
  Reference[] refs = (new ReferenceListParser(referenceList)).parseReferences();
  ...
  foreach(author; authors)
    ...
    foreach(aref; refs)
    {
      ...
      {
writefln("<h2>Chapter %d</h2>\n<div class=\"hidden\">",cidx+1);
        foreach(vidx; aref.getVerseIndices(cidx))
        {
          writefln("<p>%d:%d</p>", cidx+1, vidx+1);
          foreach(quran; qurans)
          {
            char[][] chapter = quran.chapter(cidx);
            writefln("<p><em>%s</em>: %s</p>", quran.getAuthor, chapter[vidx]);
          }
        }
writefln("</div>");
      }
...
/+
  var style = document.defaultView.getComputedStyle(divTag, pseudoElt);
alert(style);
alert(style.display);
+/
